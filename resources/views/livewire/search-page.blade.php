<div>
    <div class="container mx-auto min-h-screen py-12">
        ...
        <div class="container relative mt-12">
            ...
            <input wire:model.live="searchItem" type="text" placeholder="Search all archives..." class="rounded-lg px-10 py-2 active:border-0 text-black border-b-2 border-amber-700 placeholder:text-black placeholder:italic w-[100%] lg:w-[900px] text-lg form-control" spellcheck="false" aria-label="Search all archives" wire:keydown.escape="close()" id="searchForm" autocomplete="off" wire:debounce="6000">
        </div>
        @if($searchItem!=null)
            @if($searchForTag)
            <div class="py-2 px-10 relative" wire:transition>
                @if($entC + $colC > 0)
                Showing results tagged with <span class="text-yellow-600">{{trim($searchItem, "[]")}}</span>
                @else
                No entries tagged with <span class="text-yellow-600">{{trim($searchItem, "[]")}}</span>
                @endif
