...
class Search extends Component
{
    ...
    public function render()
    {
        ...
        if(!is_null($this->searchItem)) {
            
            $this->count = Entity::where('title', 'like', '%'.$this->searchItem.'%')->count();
            $this->results = Entity::where('title', 'like', '%'.$this->searchItem.'%')->limit(10)->get();
        }
        return view('livewire.search');
