...
class Search extends Component
{
    ...
    public function render()
    {
        ...
        if(strlen($this->searchItem) >= 1) {
            $query = Entity::where('collection_id', '=', null)->where('title', 'like', '%'.$this->searchItem.'%');
            foreach($this->columns as $column) {
                $query->orWhere($column, 'LIKE', '%'.$this->searchItem.'%');
            }
            $this->ents = $query->orderBy('title', 'asc')->limit(20)->get();
            
            $this->cols = Collection::where('collection_id', '=', null)->where('name', 'like', '%'.$this->searchItem.'%')->orWhere('description', 'like', '%'.$this->searchItem.'%')->orderBy('name', 'asc')->limit(20)->get();
        }
        return view('livewire.search');
